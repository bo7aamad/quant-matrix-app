...
try:
    ...
    # If any import or library framework crashes, lock the error log
...
if crash_traceback is None:
    ...
    class QuantApp(App):
        def build(self):
            ...
            self.balance_input = TextInput(text="1000", placeholder="Vault Balance Floor", multiline=False, size_hint_y=None, height=50, input_filter='float', font_size='18sp')
        ...
        def render_live_card(self, text):
            lbl = Label(text=text, size_hint_y=None, height=140, font_size='14sp', halign='left', valign='middle', padding=(10, 10))
...
# ==========================================
# EMERGENCY DIAGNOSTIC TERMINAL MOUNT
# ==========================================
if __name__ == "__main__":
    if crash_traceback:
        # If imports failed, Kivy base is fine, so override crash and render log on mobile screen
        from kivy.app import App
        from kivy.uix.scrollview import ScrollView
        from kivy.uix.label import Label
        class EmergencyCrashApp(App):
            def build(self):
                scroll = ScrollView()
                lbl = Label(
                    text=f"🚨 INSTANT APP CRASH OVERRIDDEN:\n\n{crash_traceback}",
                    font_size='14sp', color=(1, 0.3, 0.3, 1), size_hint_y=None, halign='left', valign='top', padding=(20, 20)
                )
                lbl.bind(size=lbl.setter('height'))
                lbl.bind(size=lbl.setter('text_size'))
                scroll.add_widget(lbl)
                return scroll
        EmergencyCrashApp().run()
    else:
        try:
            QuantApp().run()
        except Exception as global_err:
            runtime_trace = traceback.format_exc()
            from kivy.app import App
            from kivy.uix.scrollview import ScrollView
            from kivy.uix.label import Label
            class EmergencyRuntimeApp(App):
                def build(self):
                    scroll = ScrollView()
                    lbl = Label(
                        text=f"🚨 GLOBAL APP RUNTIME CRASH:\n\n{runtime_trace}",
                        font_size='14sp', color=(1, 0.3, 0.3, 1), size_hint_y=None, halign='left', valign='top', padding=(20, 20)
                    )
                    lbl.bind(size=lbl.setter('height'))
                    lbl.bind(size=lbl.setter('text_size'))
...
def fetch_macro_sentiment():
    ...
    for url in NEWS_SOURCES:
        try:
            res = requests.get(url, headers=HTTP_HEADERS, timeout=2.5)
            if res.status_code == 200:
                # Built-in high speed XML processing parsing layer
                root = ET.fromstring(res.content)
                items = root.findall('.//item')
                for item in items[:3]:
                    title_node = item.find('title')
...
def extract_advanced_metrics(ticker):
    ...
    try:
        ...
        # 2. Pure Python SMA & Bollinger Percent Engine
        last_20_closes = closes[-20:]
        sma20 = sum(last_20_closes) / 20
        variance = sum((x - sma20) ** 2 for x in last_20_closes) / 20
        std20 = math.sqrt(variance)
        ...
        # 3. Pure Python Average True Range (ATR) Engine
        tr_list = []
        for i in range(len(closes) - 15, len(closes)):
            h, l, prev_c = highs[i], lows[i], closes[i-1]
            tr = max(h - l, abs(h - prev_c), abs(l - prev_c))
        ...
        # 4. Handle Flow Signature (RVOL) safely
        rvol_str = "N/A (OTC Pair)"
        valid_vols = [v for v in volumes if v is not None and v > 0]
        if valid_vols and len(valid_vols) >= 20:
            avg_vol = sum(valid_vols[-20:]) / 20
